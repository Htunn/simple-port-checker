"""
Command Line Interface for offensive-ai.
...
██║   ██║██╔══╝  ██╔══╝  ██╔══╝  ██║╚██╗██║╚════██║██║╚██╗ ██╔╝██╔══╝
╚██████╔╝██║     ██║     ███████╗██║ ╚████║███████║██║ ╚████╔╝ ███████╗
 ╚═════╝ ╚═╝     ╚═╝     ╚══════╝╚═╝  ╚═══╝╚══════╝╚═╝  ╚═══╝  ╚══════╝[/bold red][bold white]-AI[/bold white]
...
def main():
    """offensive-ai — offensive-security toolkit for authorized red-team engagements."""
    pass
...
def mtls_check(targets, port, timeout, client_cert, client_key, ca_bundle, output, verbose, no_verify, concurrent, max_retries, retry_delay):
    ...
    \b
        # Basic mTLS check
        offensive-ai mtls-check api.example.com
        
        # Check with client certificates
        offensive-ai mtls-check api.example.com --client-cert client.crt --client-key client.key
        
        # Batch check multiple APIs
        offensive-ai mtls-check api1.com api2.com:8443 --concurrent 10 --verbose
        
        # Enterprise security audit
        offensive-ai mtls-check $(cat production-apis.txt) --output audit-results.json
        
        # Custom configuration
        offensive-ai mtls-check api.example.com --timeout 30 --max-retries 5 --retry-delay 2.0
...
def mtls_gen_cert(hostname, cert_path, key_path, days, key_size, country, organization):
    ...
    \b
        ...
        offensive-ai mtls-gen-cert api-client.com --days 90 --key-size 4096
        
        # Custom output paths
        offensive-ai mtls-gen-cert client.internal --cert-path /etc/ssl/client.crt --key-path /etc/ssl/private/client.key
        
        # Custom subject information
        offensive-ai mtls-gen-cert test.company.com --country GB --organization "ACME Corp"
...
def mtls_validate_cert(cert_path, key_path, check_expiry, check_chain, ca_bundle, verbose):
    ...
    \b
        ...
        offensive-ai mtls-validate-cert client.crt client.key --check-expiry
        
        # Validate certificate chain
        offensive-ai mtls-validate-cert client.crt client.key --check-chain --ca-bundle ca-bundle.pem
        
        # Detailed output
        offensive-ai mtls-validate-cert client.crt client.key --verbose --check-expiry
...
def hybrid_identity(targets, timeout, output, verbose, concurrent):
    ...
    \b
        ...
        offensive-ai hybrid-identity domain1.com domain2.com domain3.com
        
        # Batch check with output
        offensive-ai hybrid-identity $(cat domains.txt) --output results.json
        
        # Verbose output with DNS details
        offensive-ai hybrid-identity company.com --verbose
...
def owasp_scan(targets, deep, categories, tech_stack, format, output, severity, verbose, timeout, use_judge):
    ...
    Examples:
        ...
        offensive-ai owasp-scan example.com --deep -f pdf -o report.pdf
        
        # Scan specific categories with JSON output
        offensive-ai owasp-scan example.com -c A02,A05 -f json -o results.json
        
        # OWASP 2025 categories
        offensive-ai owasp-scan example.com -c A03_2025,A10_2025 --verbose
        
        # Multiple targets with severity filter
        offensive-ai owasp-scan site1.com site2.com --severity HIGH --verbose
    """
...
def mcp_scan(target, transport, cmd, extra_headers, timeout, no_tls_verify, output_format, output, use_judge):
    ...
    Examples:
        ...
        offensive-ai mcp-scan stdio://local --transport stdio --cmd python server.py
    """
...
def mcp_attack(target, authorized, transport, cmd, mode, extra_headers, timeout,
    ...
    Recommend running mcp-scan first to enumerate the target before attacking:
        offensive-ai mcp-scan https://mcp.example.com/mcp
        offensive-ai mcp-attack https://mcp.example.com/mcp --i-have-authorization --mode deep
    """
    if not authorized:
...
def openclaw_scan(target, port, use_tls, extra_headers, timeout, output_format, output, use_judge):
    ...
    Examples:
        ...
        offensive-ai openclaw-scan myclaw.example.com --port 18789 --tls
        offensive-ai openclaw-scan 10.0.0.5 --format json --output openclaw-report.json
        offensive-ai openclaw-scan openclaw.corp.local --header 'Authorization:Bearer mytoken'
    """
...
def openclaw_attack(target, port, use_tls, mode, extra_headers, timeout,
    ...
    Examples:
        ...
        offensive-ai openclaw-attack openclaw.corp.local --mode deep --i-have-authorization
        offensive-ai openclaw-attack 10.0.0.5 --mode deep -o attack-report.json --i-have-authorization
    """
    if not authorized:
        console.print(
...
def llm_attack(
    ...
    async def _run() -> None:
        ...
        console.print(Panel(
            f"[bold yellow]⚠  OAI-AI LLM ATTACK MODULE — AUTHORIZED USE ONLY[/bold yellow]\n\n"
            f"  Endpoint : {endpoint}\n"
            f"  Mode     : {mode}\n"
...
def k8s_scan(
    ...
    Examples:
      ...
      offensive-ai k8s-scan 10.0.0.1 --llm-judge --format json --output report.json
    """
    async def _run() -> None:
        headers: dict[str, str] = {}
        for h in extra_headers:
...
def k8s_attack(
    ...
    Examples:
      ...
      offensive-ai k8s-attack 10.0.0.1 --i-have-authorization --mode deep
      offensive-ai k8s-attack 10.0.0.1 --i-have-authorization --mode deep --output attack.json
    """
    if not authorized:
        console.print(
            "[bold red]⚠  --i-have-authorization flag is required.[/bold red]\n"
...
def auth_scan(target, protocol, extra_headers, timeout, no_tls_verify, output_format, output, use_judge):
    ...
    Examples:
        ...
        offensive-ai auth-scan https://auth.example.com --llm-judge --output result.json
    """
...
def auth_attack(target, authorized, protocol, mode, extra_headers, timeout,
    ...
    Examples:
        ...
        offensive-ai auth-attack https://auth.example.com --i-have-authorization --mode deep
        offensive-ai auth-attack https://auth.example.com --i-have-authorization --llm-judge -o report.json
    """
    if not authorized:
        console.print(
            "[bold red]⚠  --i-have-authorization flag is required.[/bold red]\n"
...
def a2a_scan(target, port, extra_headers, timeout, no_tls_verify, output_format, output, use_judge):
    ...
    Examples:
        ...
        offensive-ai a2a-scan https://agent.example.com --format json --output report.json
        offensive-ai a2a-scan https://agent.example.com --header 'Authorization:Bearer tok'
    """
...
def a2a_attack(target, authorized, mode, extra_headers, timeout, no_tls_verify,
    ...
    Recommend running a2a-scan first to enumerate the target before attacking:
        offensive-ai a2a-scan https://agent.example.com
        offensive-ai a2a-attack https://agent.example.com --i-have-authorization --mode deep
    """
    if not authorized:
        console.print("[bold red]Error:[/bold red] --i-have-authorization flag is required. "
                      "Only use this against systems you are authorized to test.")
...
def blockchain_scan(target, port, extra_headers, timeout, no_tls_verify, output_format, output, use_judge):
    ...
    Examples:
        ...
        offensive-ai blockchain-scan https://rpc.example.com --format json --output report.json
    """
...
def blockchain_contract_audit(abi_path, bytecode_path, output_format, output):
    ...
    \b
    Examples:
        offensive-ai blockchain-contract-audit --abi ./contract.json
        offensive-ai blockchain-contract-audit --abi ./contract.json --bytecode ./contract.bin
    """
...
def postman_scan(collection, environment, target_override, extra_headers, timeout,
    ...
    \b
    Examples:
        offensive-ai postman-scan my-api.postman_collection.json
        offensive-ai postman-scan collection.json -e env.json -T https://staging.example.com
        offensive-ai postman-scan collection.json --llm-judge --output report.json
    """
...
def postman_attack(collection, authorized, environment, target_override, mode, extra_headers,
    ...
    Run postman-scan first for passive reconnaissance:
        ...
        offensive-ai postman-attack collection.json --i-have-authorization --mode deep -T https://api.example.com
    """
    if not authorized:
        console.print("[bold red]Error:[/bold red] --i-have-authorization flag is required. "
                      "Only use this against systems you are authorized to test.")
